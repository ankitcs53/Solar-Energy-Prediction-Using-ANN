"""
This file preProcesses to form test data(years 2006 to 2007)
...
def doy(day,month,year):
    """
        Given a date, this method returns the day number in the year that date belongs to
    """
...
def SpacialInterpolate(filename):
    """
        Finds the values of weather variable(given in the f"ilename" for GEFS locations) 
        at the 98 mesonet sites using Catmull-Rom splines
    """
...
def ProcessData():
    ...
    # obtain processed output data with 15 features( feature segregation) 
    ...
    #get prediction values Ccorresponding to the 
    values = getSegmentedOutput()
    ...
    np.savetxt(dataPath+"inputnn1_test.csv",outputnn1,delimiter=",")
    np.savetxt(dataPath+"inputnn2_test.csv",outputnn2,delimiter=",")
    np.savetxt(dataPath+"inputnn3_test.csv",outputnn3,delimiter=",")
    np.savetxt(dataPath+"inputnn4_test.csv",outputnn4,delimiter=",")
    


def main():

    # Preprocess data for Spatial Interpolation, feature Segregation, Data Segmentation
    ProcessData()
